...
struct LightweightNowPlayingEqualizerView: NSViewRepresentable {
    ...
    func makeNSView(context _: Context) -> LightweightNowPlayingEqualizerNSView {
    ...
    func updateNSView(_ nsView: LightweightNowPlayingEqualizerNSView, context _: Context) {
    ...
    static func dismantleNSView(_ nsView: LightweightNowPlayingEqualizerNSView, coordinator _: ()) {
        nsView.stop()
    }
}
...
final class LightweightNowPlayingEqualizerNSView: NSView {
    ...
    func setColors(_ colors: [NSColor]) {
        guard barColors.count != colors.count || zip(barColors, colors).contains(where: { !$0.0.isEqual($0.1) })
        else { return }
        barColors = colors

        let cgColors: [CGColor] = if colors.isEmpty {
            [NSColor.white.cgColor, NSColor.white.cgColor]
        } else if colors.count == 1 {
            [colors[0].cgColor, colors[0].cgColor]
        } else {
            colors.map(\.cgColor)
        }
